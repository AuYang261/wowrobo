# python camera/orb_camera_server.py --mode multi
...
def display_stream():
    ...
    while True:
        ...
        processed_frames = {
            "color": process_color(frames.get_color_frame()),
        ...
        try:
            left = process_ir(
                frames.get_frame(OBFrameType.LEFT_IR_FRAME).as_video_frame()
            )
            right = process_ir(
                frames.get_frame(OBFrameType.RIGHT_IR_FRAME).as_video_frame()
            )
...
def start_multi_server(host, port):
    ...
    def rgb_generator():
        while True:
            with metadata_lock:
                if (
                    global_processed_frames is None
                    or global_processed_frames.get("color") is None
                ):
                    continue
    ...
    def depth_generator():
        while True:
            with metadata_lock:
                if (
                    global_processed_frames is None
                    or global_processed_frames.get("depth") is None
                ):
                    continue
    ...
    def rgb_stream():
        return Response(
            rgb_generator(), mimetype="multipart/x-mixed-replace; boundary=dictboundary"
        )
    ...
    threading.Thread(
        target=frame_generator, args=(pipeline, imu_pipeline), daemon=True
    ).start()
    app.run(host=host, port=port, threaded=True, use_reloader=False)
...
def main():
    ...
    parser.add_argument(
        "--mode", type=str, default="multi", help="Video source (default: rgb)"
    )
